...
def plot_metric_curve(df: pd.DataFrame, metric: str,
    ...
    _style_ax(ax,
              xlabel="Cold-start severity (% user history retained)",
              ylabel=f"{metric.upper()}@{top_k}",
              title=f"Degradation Curve — {metric.upper()}@{top_k}")
    plt.tight_layout()
...
def print_summary_table(df: pd.DataFrame):
    for metric in ["recall", "ndcg"]:
        ...
        print(f"\n{'─' * 55}")
        print(f"  {metric.upper()}@10 across cold-start levels")
        print(f"{'─' * 55}")
