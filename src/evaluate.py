...
Works with all three models:
  ALSModel         (src/model_als.py)
  SemanticModel    (src/model_semantic.py)
...
from src.preprocess import simulate_cold_start, COLD_START_LEVELS

log = logging.getLogger(__name__)

TOP_K       = 10
RESULTS_DIR = Path("results")
MIN_ALPHA   = 10   # same as teammate's alpha parameter
...
def evaluate_model(model_name: str,
    ...
    for k_pct in cold_start_levels:
        ...
        for uid in tqdm(users, desc=f"{model_name} K={k_pct}%", leave=False):
            ...
            gt_set       = set(gt_rows["item_idx"].tolist())
            ...
            if als_model is not None:
                ...
                from src.model_dropoutnet import fold_in_user
                kwargs["als_user_vec"] = (
