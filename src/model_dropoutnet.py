...
def fold_in_user(seed_item_indices: list[int],
    ...
    cu = 1.0 + alpha * np.array(seed_ratings)      # (K,)
...
class _DropoutNetNN(nn.Module):
    ...
    def forward(self, u_pref, pos_i_pref, pos_i_cont, neg_i_pref, neg_i_cont):
        user_repr = self.encode_user(u_pref, pos_i_pref)
...
class DropoutNetModel:
    ...
    def train(self, train_df: pd.DataFrame,
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch in loader:
                ...
                u_pref, i_pref, i_cont, n_pref, n_cont = batch
                pos_score, neg_score = self.nn(u_pref, i_pref, i_cont,
                                               n_pref, n_cont)
    ...
    def recommend(self, seed_item_indices: list[int],
        ...
        if als_user_vec is None or len(seed_item_indices) == 0:
